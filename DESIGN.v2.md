---
name: Shreenivas Joshi — the datasheet (/v2)
description: >-
...
# Design System: the datasheet (`/v2`)

> **Scope.** This document describes the datasheet — the surface at `/v2`,
> built from `src/app/v2/page.tsx` and `src/components/*`. It was the front
> page when it was written and is still built and maintained to every rule
> below.
>
> The front page at `/` is a different world and has its own document:
> `DESIGN.md`. The two surfaces deliberately share no tokens — only the content
> in `src/lib/content.ts`. Nothing here is binding on that page, and nothing
> there is binding on this one.
